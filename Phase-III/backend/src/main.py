...
from .api.chat_exceptions import ChatException
from .api.chat_schemas import ChatErrorResponse, ErrorResponseWrapper
...
def create_app() -> FastAPI:
    ...
    async def chat_exception_handler(request: Request, exc: ChatException):
        """Handle chat API exceptions."""
        request_id = str(uuid.uuid4())
        logger.warning(
            f"Chat Exception [{request_id}]: {exc.error_code} - {exc.error_message}",
    ...
    # [Task]: T329, T330, T331, [From]: specs/004-ai-chatbot/spec.md#Requirements
    # Include chat routers for conversation and message management
